...
{
    private static final String TAG = "CertVer"; 
            ...
            throws CertificateVerificationException
    {
        try
        {
            ...
            if (downloadSize > 0)
            {
                Log.i(TAG, "CA issuers: " + downloadSize + " downloaded certificate(s) are new");
            }
            ...
            Log.i(TAG, "Certification chain verified successfully up to this root: " +
            ...
            GeneralSecurityException, OCSPException
    {
        ...
        if (ocspURL != null)
        {
            ...
            catch (IOException ex)
            {
                ...
                Log.w(TAG, "IOException trying OCSP, will try CRL", ex);
                Log.w(TAG, "Certificate# to check: " + cert.getSerialNumber().toString(16));
                //CRLVerifier.verifyCertificateCRLs(cert, signDate, additionalCerts);
            }
            catch (OCSPException ex)
            {
                // happens with QV_RCA1_RCA3_CPCPS_V4_11.pdf
                Log.w(TAG, "OCSPException trying OCSP, will try CRL", ex);
                Log.w(TAG, "Certificate# to check: " + cert.getSerialNumber().toString(16));
                //CRLVerifier.verifyCertificateCRLs(cert, signDate, additionalCerts);
            }
        }
        ...
        {
            Log.i(TAG, "OCSP not available, will try CRL");
    ...
    public static boolean isSelfSigned(X509Certificate cert) throws GeneralSecurityException
    {
        ...
        catch (SignatureException ex)
        {
            ...
            Log.d(TAG, "Couldn't get signature information - returning false", ex);
            return false;
        }
    ...
    public static Set<X509Certificate> downloadExtraCertificates(X509Extension ext)
    {
        ...
        {
            Log.w(TAG, "ASN1Sequence expected, got " + asn1Prim.getClass().getSimpleName());
            return resultSet;
        }
        ...
        while (objects.hasMoreElements())
        {
            ...
            try
            {
                Log.i(TAG, "CA issuers URL: " + urlString);
                ...
                Log.i(TAG, "CA issuers URL: " + altCerts.size() + " certificate(s) downloaded");
            }
            catch (IOException ex)
            {
                Log.w(TAG, urlString + " failure: " + ex.getMessage(), ex);
            }
        ...
        Log.i(TAG, "CA issuers: Downloaded " + resultSet.size() + " certificate(s) total");
        return resultSet;
    }
            ...
            throws RevokedCertificateException, IOException, OCSPException, CertificateVerificationException
    {
        ...
        Log.i(TAG, "OCSP check successful");
        ...
        if (ocspResponderCertificate.getExtensionValue(OCSPObjectIdentifiers.id_pkix_ocsp_nocheck.getId()) != null)
        {
            ...
            Log.i(TAG, "Revocation check of OCSP responder certificate skipped (id-pkix-ocsp-nocheck is set)");
            return;
        }

        if (ocspHelper.getCertificateToCheck().equals(ocspResponderCertificate))
        {
            Log.i(TAG, "OCSP responder certificate is identical to certificate to check");
            return;
        }

        Log.i(TAG, "Check of OCSP responder certificate");
        ...
        Log.i(TAG, "Check of OCSP responder certificate done");
    }
}
