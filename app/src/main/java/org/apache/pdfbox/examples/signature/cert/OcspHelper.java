...
public class OcspHelper
{
            ...
            throws OCSPException, RevokedCertificateException, IOException
    {
        ...
        if (basicResponse != null)
        {
            ...
            if (status instanceof RevokedStatus)
            {
                ...
                Log.i(TAG, "The certificate was revoked after signing by OCSP " + ocspUrl + 
    ...
    private byte[] getKeyHashFromCertHolder(X509CertificateHolder certHolder) throws IOException
    {
        ...
        {
            // should not happen
            Log.e(TAG, "SHA-1 Algorithm not found", ex);
            return new byte[0];
        }
    }
        ...
        try
        {
                ...
                responseCode == HttpURLConnection.HTTP_SEE_OTHER)
            {
                    ...
                    urlString.substring(7).equals(location.substring(8)))
                {
                    ...
                    Log.i(TAG, "redirection to " + location + " followed");
                    return performRequest(location);
                }
                else
                {
                    Log.i(TAG, "redirection to " + location + " ignored");
                }
            }
    ...
    public void verifyRespStatus(OCSPResp resp) throws OCSPException
    {
        ...
        if (resp != null)
        {
            ...
            case OCSPResponseStatus.INTERNAL_ERROR:
                ...
                Log.e(TAG, "An internal error occurred in the OCSP Server!");
                break;
            case OCSPResponseStatus.MALFORMED_REQUEST:
                ...
                Log.e(TAG, "Your request did not fit the RFC 2560 syntax!");
                break;
            case OCSPResponseStatus.SIG_REQUIRED:
                statusInfo = "SIG_REQUIRED";
                Log.e(TAG, "Your request was not signed!");
                break;
            case OCSPResponseStatus.TRY_LATER:
                statusInfo = "TRY_LATER";
                Log.e(TAG, "The server was too busy to answer you!");
                break;
            case OCSPResponseStatus.UNAUTHORIZED:
                statusInfo = "UNAUTHORIZED";
                Log.e(TAG, "The server could not authenticate you!");
                break;
            case OCSPResponseStatus.SUCCESSFUL:
                break;
            default:
                statusInfo = "UNKNOWN";
                Log.e(TAG, "Unknown OCSPResponse status code! " + status);
            }
        }
